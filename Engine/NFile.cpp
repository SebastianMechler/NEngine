...
namespace Nully
{
  ...
  NResult NFile::Read(void * a_buffer, const size_t a_bufferSize) const
  {
    ...
    if (readBytes != 1) // fread returns how many elements read, since only 1 element is read, ...
    {
      // file-end maybe reached...
      return NResult::FileReadError;
    }
    else
    {
      return NResult::Success;
  ...
  NResult NFile::ReadLine(char* a_buffer, const size_t a_bufferSize) const
  {
    ...
    return NResult::FileReadLineError;
  }
  ...
  uint32_t NFile::GetFileSize(const char * a_path)
  {
    if (a_path == nullptr)
    {
      return 0;
    ...
    if (size.QuadPart > 0)
    {
      return static_cast<uint32_t>(size.QuadPart);
    }
    //return size;
